...
ただし **「模式線」は地理的整合性を無視してよいという意味ではない。**

- 海路・河川航路を線で描く場合、史料上の根拠がない陸地横断をさせない
- 長距離街道を少数の都市間直線だけで近似し、峠・河川・海岸・宿場など経路を規定する地形条件を失わせない
- 信頼できる歴史GISのroute geometryが利用できる場合は、手描きの直線近似よりそちらを優先する
- 一定の固定線形を持たない海運・交易ネットワークは、一本のLineStringへ押し込まず、寄港地の点群・複数経路・海域コリドーなど、史料の確度に合った表現へ落とす
- 史料で確認できるのが経由順だけの場合は、waypoint由来の概略線であることを明示し、測量済みの旧道中心線のように見せない
...
- 手描きrouteが利用可能な歴史GISより不必要に粗くなっていないか
- 境界の解釈が史料の意味を変えていないか
