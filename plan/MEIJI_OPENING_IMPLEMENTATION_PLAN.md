...
- A3「戊辰戦争の空間展開」を実装した。鳥羽・伏見、江戸、長岡、会津若松、箱館・五稜郭の代表点のみを示し、進軍路・戦線・藩境のLineStringは作らない。Data Audit / Style Audit は passed、Human Visual Audit は GitHub Pages 上の確認待ちで `pending-human` とする。
...
- JH10は本文・Data/Style Auditまで実装済みだが、A3 Human Visual Audit 完了までは地図実装を完成扱いにしない。公開面には規約どおり「監査中です」を表示して実画面確認できる状態にする。
