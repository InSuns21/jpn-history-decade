...
- A2 Data Auditでは、横浜が東海道上の神奈川宿ではなく横浜道で接続されたこと、長崎が長崎街道で小倉方面へ接続したこと、箱館が開港以前から北前船・西廻り海運へ接続していたことを公的資料で確認した。
- 現代道路や現在の航路を歴史線として流用せず、史料で確認した起終点・主要経由方向だけをサイト側で模式線化し、すべて geometryConfidence=schematic または approximate とした。
- A2「1859年の開港場と既存国内交通軸（模式図）」を実装した。横浜・長崎・箱館を1859年の実開港場として表示し、江戸・日本橋、神奈川宿、大坂、小倉を交通結節点として補助表示する。
- 東海道・長崎街道・西廻り海運・横浜道を線種と色で区別し、模式線は破線に統一した。Data Audit / Style Audit は passed。
- Human Visual Audit は pending-human。GitHub Pages上に「監査中です」を表示して公開し、desktop / tablet-touch / mobile / zoom別にラベル重なり、線の識別性、popup、pan / zoomを確認する。
